...
if command -v flatpak &> /dev/null; then
    flatpak list --app --columns=application > "$FLATPAK_PACKAGE_LIST"
    ...
    echo "  - Flatpak packages: $FLATPAK_PACKAGE_LIST ($(wc -l < "$FLATPAK_PACKAGE_LIST") packages)"
else
    echo "Flatpak not installed, skipping Flatpak package export"
    echo "Package lists exported successfully!"
    echo "  - Official packages: $PACKAGE_LIST ($(wc -l < "$PACKAGE_LIST") packages)"
    echo "  - AUR packages: $AUR_PACKAGE_LIST ($(wc -l < "$AUR_PACKAGE_LIST") packages)"
fi
